...
class PersianDate():
    ...
    def get_greg_date(self):
        cal = Calverter()
        jd = cal.jalali_to_jd(self.year, self.month, self.day)
        greg = cal.jd_to_gregorian(jd)
        return datetime.date(greg[0], greg[1], greg[2])

    def __unicode__(self):
        return "{0} {1} {2}".format(self.day, JALALIMONTH[self.month - 1][1], self.year)
...
class Calverter:
    def __init__(self):
        ...
        self.JALALI_WEEKDAYS = ("یکشنبه", "دوشنبه", "سه‌شنبه", "چهارشنبه", "پنجشنبه", "جمعه", "شنبه")
